...
function modifyQueue(interaction) {
  ...
  if (!memberChannelId) {
    return interaction.client.say.errorMessage(interaction, "Vous devez d'abord rejoindre un canal vocal !");
  }

  if (memberChannelId !== botChannelId) {
    return interaction.client.say.warnMessage(interaction, "Vous devez être dans le même canal vocal que moi !");
  }
