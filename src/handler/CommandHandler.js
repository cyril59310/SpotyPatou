...
module.exports = async function loadCommands(bot) {
  ...
  for await (const file of commandFiles) {
    ...
    const data = {
      name: command.name,
      description: command?.description ?? "Description vide",
  ...
  console.log(chalk.green('[๖̶ζ͜͡Patoûûû༻#8321]') + chalk.cyan(' Merci d utilisé SpotiTou'))
  console.log(chalk.red('=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+='))
  console.log(chalk.green('Bot: ') + chalk.cyan(`${bot.user.tag}`))
  console.log(chalk.green('Status: ') + chalk.cyan('Initialisé'))
  console.log(chalk.red('=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+='))
